...
type ExtendChildRoute<ParentT, ChildR> =
  ChildR extends Route<[]>
    ? Route<ParentT> /* Don't merge empty route params */ & {
        [k in keyof Without<ChildR, RouteKeys>]: ExtendChildRoute<ParentT, ChildR[k]>
      }
    : ChildR extends Route<infer ChildT>
      ? Route<ParentT & ChildT> & {
          [k in keyof Without<ChildR, RouteKeys>]: ExtendChildRoute<ParentT & ChildT, ChildR[k]>
        }
      : ChildR
...
function createRoute(
  ...
  children: Record<string, Route<any>>,
): any {
  ...
  for (const key of _keys) {
    ...
    if (i === -1) {
      throw new Error(
        `Param name found in route path string but not in param handlers: ':${key.name}'\n  for route path: ${_path}`,
      )
    }
  ...
  const linkFn = function (..._params: any): string {
    return _c(..._params)
  }
...
export function route<
  P extends ParamType,
  Children extends { [_: string]: Route<any> } | unknown = unknown,
>(path: string, params: undefined, children: Children): Route<[]> & Children
